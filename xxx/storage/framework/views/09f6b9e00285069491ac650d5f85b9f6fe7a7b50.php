    ...
    $.getJSON('data/'+uid, function(mydata) {
        if (mydata) {
            ...
            $.each(mydata, function(index, page) {
                ...
                if(page.isdef=='1')
                {
                    myArray.push([ dem, page.pagename, page.pageid, "<a target='_blank' href='http://fb.com/" + page.pageid + "'>View Fanpage</a>", "<div id='btn"+page.pageid+"'><button type='button' class='btn btn-primary' style='width:90px'>Default</button></div>"]);
                }else{
                    myArray.push([ dem, page.pagename, page.pageid, "<a target='_blank' href='http://fb.com/" + page.pageid + "'>View Fanpage</a>", "<div id='btn"+page.pageid+"'><button type='button' class='btn btn-default' style='width:90px' onclick='changeDef("+page.pageid+")'>Set Default</button></div>"]);
                }
                dem++;
            });
