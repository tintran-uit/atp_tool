    ...
    <div class="row">
        ...
        <div class="modal fade" id="myModal" role="dialog">
            ...
            <div class="modal-footer">
              ...
              <a type="button" onclick="postFB();" class="btn btn-primary">Post</a>
          </div>
      </div>
...
<script type="text/javascript">
    ...
    $.getJSON(url, function(data) {
        if (data.name) {
            ...
            $('z').text(category);
            $('span.badgediv.profile-data-title-danger').text(talk);
            ...
            $.getJSON(urltable, function(mydata) {
                ...
                if (data.data) {
                    ...
                    $.each(data.data, function(index, mydata) {
                        ...
                        if (isvideo != '') {
                            myArray.push([dem, isvideo, mesz, likes + ' / ' + comz, created.substring(0, 10), '<a onclick="schedule(' + dem + ',\''+ idz + '\',\'' + picz + '\')" class="btn btn-primary" data-toggle="modal" data-target="#myModal" style="width:90px">Schedule</a><br/><a target="_blank" class="btn btn-default" href="http://fb.com/' + idz + '" style="margin-top:5px; width:90px">Preview</a>']);
                            dem++;
                        }
                    });
    ...
    function postFB() {
        ...
        if(checktime>time)
        {
            if(checktime>time+400)
            {
                toastr.warning("Scheduled publish time must more 10 minutes!","Post now!");
            }
            ...
            $.post("https://graph.facebook.com/" + mypage + "/videos?method=post", datapost,
                function(response) {
                    ...
                    if (response && !response.error) {
                        toastr.success("View your post: <a target='_blank' href='http://fb.com/" + response.id + "'> click link</a>", "Successful post");
                    }else{
