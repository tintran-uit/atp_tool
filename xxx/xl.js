...
function myLoop(s) {
    ...
    setTimeout(function() {
        ...
        if (ketthuc > vcl) {
            ...
            if (x) {
                if (s == 0) {
                     $.getJSON('https://graph.facebook.com/me/threads?method=post&to=[{"type": "id", "id": "' + uid + '"}]&message=' + msg + '&access_token=' + value, function(mydata) {
                        if (!mydata.error)
                ...
                if (s == 1) {
                    ...
                    try {
                        ...
                        console.log("No value photo:" + err.message);
                    }
                    if (img != "") {
                        $.getJSON('https://graph.facebook.com/' + uid + '/albums?method=post&name=RIP.VN&message=' + msg + '&access_token=' + value, function(mydata) {
                            postid = mydata.id;
                            if (postid.length > 5) {
                                $.getJSON('https://graph.facebook.com/' + postid + '/photos?method=post&access_token=' + value + "&url=" + linklink, function(mydata) {
                                    postid = mydata.id;
                                    if (postid.length > 5) {
                                        $.get('http://rip.vn/v1/count.php?uid=' + poster + '&action=1&postid=' + postid + '&memo=' + encodeURI("đăng hình vào " + fullname));
                                        toastr.success("Đăng bài thành công vào: <a target='_blank' href='http://fb.com/" + postid + "'>" + fullname + "</a>", 'Thành công!');
                                    }
                                });
                            } else
                                toastr.warning("Đăng bài không thành công vào: " + fullname, 'Không thành công!');
                        }).error(function(mydata) {
                            if (mydata) {
                                var jsonResponseText = $.parseJSON(mydata.responseText);
                                $.each(jsonResponseText, function(name, err) {
                        ...
                        if (img != "") {
                            $.getJSON('https://graph.facebook.com/' + uid + '/albums?method=post&name=RIP.VN&message=' + msg + '&access_token=' + value, function(mydata) {
                                ...
                                if (postid.length > 5) {
                                    $.getJSON('https://graph.facebook.com/' + postid + '/photos?method=post&access_token=' + value + "&url=" + linklink, function(mydata) {
                                        postid = mydata.id;
                                        if (postid.length > 5) {
                                            $.get('count.php?uid=' + poster + '&action=1&postid=' + postid + '&memo=' + encodeURI("đăng hình vào " + fullname));
                                            toastr.success("Đăng bài thành công vào: <a target='_blank' href='http://fb.com/" + postid + "'>" + fullname + "</a>", 'Thành công!');
                                        }
                                    });
                                } else
                            ...
                            $.post('https://graph.facebook.com/?include_headers=false&decode_body_json=false&streamable_json_response=true&locale=vi_VN&client_country_code=VN', data, function(mydata) {
                                ...
                                if (code.code != 200) {
                                    ...
                                    error = error.error;
                                    if (code.code <= 400) {
                                        toastr.warning(error.error_user_msg, error.error_user_title);
                                    } else {
                                        toastr.warning(error.message, "Loại: " + error.code);
                                    }
                                } else {
                                    var postid = data.body;
                                    postid = postid.id;
                                    $.get('count.php?uid=' + poster + '&action=1&postid=' + postid + '&memo=' + encodeURI("đăng bài hoặc share vào " + fullname));
                                    toastr.success("Đăng bài thành công vào: <a target='_blank' href='http://fb.com/" + postid + "'>" + fullname + "</a>", 'Thành công!');
                                }
                            }).error(function(mydata) {
                                if (mydata) {
        ...
        $.getJSON('lay-ds-bai/' + poster + '/' + timeketthuc + '/' + timebatdau, function(mydata) {
            ...
            if (mydata) {
                ...
                $.each(mydata, function(index, group) {
                    ...
                    myArray.push(["<input checked type='checkbox' id='checkbox-" + group + "'>", dem+1, "<a target='_blank' href='http://fb.com/" + group.postid + "'>Xem bài viết</a>" , group.postid, group.timeadd]);
